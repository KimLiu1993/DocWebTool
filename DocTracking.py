...
#读取SQL代码
with open(common.cur_file_dir()+'\\sql\\cutting_code.sql','r') as cutting_code:
    cutter_code = cutting_code.read()

with open(common.cur_file_dir()+'\\sql\\first mapper code.sql','r') as first_mapping_code:
    first_mapper_code = first_mapping_code.read()

with open(common.cur_file_dir()+'\\sql\\last mapper code.sql','r') as last_mapping_code:
    last_mapper_code = last_mapping_code.read()

with open(common.cur_file_dir()+'\\sql\\charting_code.sql','r') as charting_code:
    charter_code = charting_code.read()

with open(common.cur_file_dir()+'\\sql\\link code.sql','r') as link_code:
    linker_code = link_code.read()

def get_processid(connection_string,docid):
    code = '''
        select ProcessId
...
def run(connection_string,processid):
    total_result = get_log(connection_string,processid)
